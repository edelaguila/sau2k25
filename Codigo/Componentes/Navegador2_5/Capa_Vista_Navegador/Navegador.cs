...
//clase estatica para manejar la referencia al formulario de Consulta Simple
                    ...
                    if (sEstadoOK == "" && iCorrecto == 0) // Si el estado de la sTablaPrincipal es correcto
                    {
                        ...
                        else
                        {
                            if (NumeroAlias() == logic.ContarCampos(sTablaPrincipal)) // Verifica si el número de alias coincide con el número de campos de la sTablaPrincipal
                            {
                                ...
                                ReposicionarComponentesDinamicos(); // Ajusta posiciones iniciales según tamaño actual
                                this.Resize -= Navegador_Resize;
                                this.Resize += Navegador_Resize; // Reposicionar dinámico al redimensionar
       ...
       public void AsignarComboConTabla(string sTablaPrincipal, string sCampoClave, string sCampoDisplay, int iModo)
        ...
        void CreaComponentes()
        {
            ...
            int iIndex = 0;
            int iFin = logic.ContarCampos(sTablaPrincipal); // usar solo columnas reales
            int iNumeroCampos = 1; // Contador de campos
        ...
        // Reposiciona etiquetas y campos creados dinámicamente en una cuadrícula fluida al cambiar tamaño
        private void ReposicionarComponentesDinamicos()
        {
            // parámetros responsivos
            int margenIzquierdo = 50;
            int margenSuperior = 150;
            int espacioVertical = 60;
            int anchoColumna = 300; // ancho aproximado por columna
            int maxColumnas = Math.Max(1, (this.Width - margenIzquierdo) / anchoColumna);

            // recolectar pares Label-control por orden de aparición
            var pares = new List<(Label label, Control campo)>();
            foreach (Control c in this.Controls)
            {
                if (c is Label && !string.IsNullOrEmpty(c.Name) && c.Name.StartsWith("lb_"))
                {
                    string nombreCampo = c.Name.Length > 3 ? c.Name.Substring(3) : string.Empty;
                    ...
                    try
                    {
                        campo = this.Controls.Find(nombreCampo, false).FirstOrDefault();
                    }
                    catch
                    {
                        campo = null;
            ...
            {
                int col = indice % maxColumnas;
                int fila = indice / maxColumnas;
                int x = margenIzquierdo + col * anchoColumna;
                int y = margenSuperior + fila * espacioVertical;

                par.label.Location = new Point(x, y);
                par.campo.Location = new Point(x, y + par.label.Height + 5);
                indice++;
            }
        }
        ...
        {
            ReposicionarComponentesDinamicos();
        }
        ...
        string CrearDelete(string sNombreTabla, bool esTablaAdicional)
        {
            ...
            if (!esTablaAdicional)
            {
                // 1) Intentar obtener el nombre real de la PK desde la BD
                string sNombrePk = logic.ObtenerClavePrimaria(sNombreTabla);

                if (!string.IsNullOrEmpty(sNombrePk))
                {
                    foreach (Control componente in Controls)
                    {
                        if ((componente is TextBox || componente is DateTimePicker || componente is ComboBox)
                            && string.Equals(componente.Name, sNombrePk, StringComparison.OrdinalIgnoreCase)
                            && !string.IsNullOrEmpty(componente.Text))
                        {
                            sWhereQuery += sNombrePk + " = '" + componente.Text + "' ";
                            claveAsignada = true;
                            Console.WriteLine($"Clave primaria detectada: {sNombrePk} = {componente.Text}");
                            break;
                        }
                    }
                }

                // 2) Fallback: por compatibilidad con formularios antiguos que nombran controles con 'pk'
                if (!claveAsignada)
                {
                    foreach (Control componente in Controls)
                    {
                        if ((componente is TextBox || componente is DateTimePicker || componente is ComboBox)
                            && componente.Name.IndexOf("pk", StringComparison.OrdinalIgnoreCase) >= 0
                            && !string.IsNullOrEmpty(componente.Text))
                        {
                            sWhereQuery += componente.Name + " = '" + componente.Text + "' ";
                            claveAsignada = true;
                            Console.WriteLine($"Clave primaria inferida por nombre: {componente.Name} = {componente.Text}");
                            break;
                        }
                    }
                }
        ...
        private void Btn_Eliminar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (drRespuestaEliminar == DialogResult.Yes)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(sQueryPrincipal))
                    {
                        throw new InvalidOperationException("No se pudo construir la sentencia de eliminación. Verifique que el control de la PK tenga valor.");
                    }
    ...
    private void Btn_Salir_Click(object sender, EventArgs e)
		...
		private void Btn_Ayuda_Click(object sender, EventArgs e)
        {
			...
			// Buscar hacia arriba desde la ubicación del ejecutable hasta encontrar el CHM
			string sPathAyuda = sFindChmUpwards(sExecutablePath, nombreArchivoConExtension);
			/*******************************************************************/
           ...
           // string sPathAyuda = sfunFindFileInDirectory(sProjectPath, "AyudaNavegador.chm");
			...
			{
				MessageBox.Show("❌ ERROR: No se encontró el archivo de ayuda '" + nombreArchivoConExtension + "' buscando desde la carpeta de la aplicación.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
        }
        ...
        // Busca una carpeta con nombre específico hacia arriba en el árbol, comenzando en startPath
        private string sFindFolderUpwards(string startPath, string folderName)
        {
            try
            {
                DirectoryInfo dir = new DirectoryInfo(startPath);
        ...
        // Busca un archivo .chm hacia arriba en el árbol, comenzando en startPath
        private string sFindChmUpwards(string startPath, string chmFileName)
        {
            try
            {
                DirectoryInfo dir = new DirectoryInfo(startPath);
        ...
        //Busca un archivo de reporte (.rpt) dentro de un directorio y sus subcarpetas.
        private string sfunFindRptInDirectory(string sDirectory, string sRptFileName)
        {
            try
            {
                if (Directory.Exists(sDirectory))
                {
                    ...
                    return sFiles.FirstOrDefault(file => Path.GetFileName(file).Equals(sRptFileName, StringComparison.OrdinalIgnoreCase));
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("⚠️ Error al buscar el reporte: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            return null;
        }
